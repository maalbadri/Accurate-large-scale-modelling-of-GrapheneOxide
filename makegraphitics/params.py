...
class Parameterise(object):
    ...
    def match_angles(self, angle_types):
        ...
        for i in range(len(angle_types)):
            ...
            if found == 0:
                ...
                for j in range(3):
                    if new_atoms[j] == 48:
                        new_atoms[j] = 47  # Try alkene carbon instead of aromatic
                    if new_atoms[j] == 49:
                        new_atoms[j] = 46  # Try alkene H
                angle_coeffs, found = search_angles(angle_data, new_atoms, angle_coeffs)
                if found == 1:
                    print found, [a1, a2, a3], new_atoms, "sub aromatic -> alkene"
            if found == 0:
                new_atoms = atoms[:]
                if new_atoms[1] == 48:
                    new_atoms[1] = 47
                    if new_atoms[0] == 13:
                        ...
                        new_atoms[2] = 47
                    angle_coeffs, found = search_angles(
                        angle_data, new_atoms, angle_coeffs
                    )
                    if found == 1:
                        print found, [a1, a2, a3], new_atoms, "made 13 -> 47 sub"
            if found == 0:
                azide_angle = [56, 13, 48]
                if atoms == azide_angle or atoms == list(reversed(azide_angle)):
                    new_atoms = [56, 13, 3]
                    angle_coeffs, found = search_angles(
                        angle_data, new_atoms, angle_coeffs
                    )
                    if found == 1:
                        print found, [a1, a2, a3], new_atoms, "azide sub 48 -> 3"
    ...
    def match_dihedrals(self, dihedral_types):
        ...
        for i in range(len(dihedral_types)):
            ...
            if found == 0:
                if atoms[1:4] == [13, 13, 56] or atoms[0:3] == [56, 13, 13]:
                    atoms = [0, 13, 13, 3]
                    dihedral_coeffs, found = search_dihedrals(
                        dihedral_data, atoms, dihedral_coeffs
                    )
                    if found != 0:
                        print found, [a1, a2, a3, a4], atoms, "made azide sub 1"

            if found == 0:
                azide2 = [47, 13, 56, 18]
                if atoms == azide2 or atoms == list(reversed(azide2)):
                    atoms = [13, 13, 56, 18]
                    dihedral_coeffs, found = search_dihedrals(
                        dihedral_data, atoms, dihedral_coeffs
                    )
                    if found != 0:
                        print found, [a1, a2, a3, a4], atoms, "made azide sub 2"
